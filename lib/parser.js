	...
	var decoder = {
		MultiPolygon: function (coordinate, encodeOffset) {
			...
			for (var j = 0, N = coordinate.length; j < N; j++) {
				result.push(decodeCoordinate(coordinate[j], encodeOffset[j]))
			}
...
Region.prototype.getBoundingRect = function () {
	...
	for (var i = 0, L = this.contours.length; i < L; i++) {
		var boundingRect = getBoundingRect(this.contours[i])
...
module.exports = function (geoJSON) {
	...
	return features.map(function (feature) {
		...
		var coordinates = geometry.coordinates
		console.log(coordinates.slice())

		if (geometry.type === 'MultiPolygon') {
			coordinates.forEach(function (coordinate, index) {
				var result = coordinates[index] = []
